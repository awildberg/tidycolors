...
new_pillar_shaft_simple <- function(formatted, ..., width = NULL, align = "left",
  ...
  ## Wildberg added: Sys.time() "2025-05-05 22:17:23 CEST"
  # orig: -
  formatted_pre <- formatted
  if(all(grepl("TRUE|FALSE", formatted))) {
    formatted_pre <- ifelse(formatted_pre == "TRUE", crayon_green(as.character(formatted_pre)), crayon_red(as.character(formatted_pre)))
  } #else {formatted_pre <- crayon_white(as.character(formatted_pre))}
  ...
  new_pillar_shaft(
    ## Wildberg added: Sys.time() "2025-05-05 22:17:57 CEST"
    # orig: list(formatted),
    list(formatted_pre),
    #list(forma),
    ...,
